...
import { Vehicle } from '@/api/entities'; // New import for Vehicle
// Removed User import
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Add small delay between calls to mitigate rate limiting
    ...
    async function fetchMessages() {
      if (selectedDealerId) {
        ...
        try {
          ...
          // Don't clear messages on error, keep existing ones if any, or leave empty if first load failed.
        } finally {
          setIsLoading(false);
        }
      } else {
  ...
  const extractPriceFromMessage = (content) => {
    ...
    if (prices && prices.length > 0) {
      ...
      if (numericPrice > 1000) { // Only consider reasonable car prices
        return numericPrice;
      }
    }
    return null;
  ...
  const handleMessageSubmit = async (content, direction, channel = 'app') => {
    ...
    try {
      ...
      const messageData = {
        content,
        deal_id: currentDeal.id, // Add the required deal_id
        dealer_id: selectedDealerId,
        direction,
        channel, // Add channel support
        is_read: true,
  ...
  const handleSendMessage = async () => {
    await handleMessageSubmit(newMessage, 'outbound', 'app'); // Pass channel 'app'
    setNewMessage('');
  };
  ...
  const handleAISuggestion = async () => {
    ...
    try {
      ...
      setIsSuggestionModalOpen(true); // Open the modal after suggestions are fetched
    } catch(e) {
      console.error(e);
  ...
  // Removed handleSmsSent function
            ...
            <QuickActions 
              ...
              onAction={(action, data) => {
                if (action === 'send_message') {
                  handleMessageSubmit(data.message, 'outbound', 'app'); // Pass channel 'app'
                }
              }}
            />
          )}

          {/* Message Input - Email Only */}
          <div className="bg-white border-t border-slate-200 p-4">
              ...
              <Dialog open={isSuggestionModalOpen} onOpenChange={setIsSuggestionModalOpen}>
                <DialogTrigger asChild>
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={handleAISuggestion} // This triggers the suggestion fetch
            ...
            <div className="flex gap-2">
              <Textarea
                ...
                rows={2}
              />
              <div className="flex flex-col gap-2"> {/* Added flex container for buttons */}
                <Button 
                  size="icon" 
